---
title: "Linear Discriminant Analysis - ACP"
author: "Solène Pety"
date: "27/04/2021"
output:
  html_document:
...
files=c("circadian_rythm.txt","photo.txt","DEGDROUGHT.txt",
        ...
        "DEGHEAVY.METAL.txt","DEGNITROGEN.txt","DEGOXYDATIVE.STRESS.txt",
        "DEGTEMPERATURE.txt","DEGUV.txt")

name=c("Circadian rythm","Photosynthesis","Drought",
       "Gamma","Other abiotic","Salt",
       "Heavy Metal","Nitrogen","Oxydative Stress","Temperature","UV")
...
# dim=30
 ...
 Coord_pred=cbind(Stress=set_test$vec,Coord_pred)
  ...
  #logset_test=Coord_pred
  #logdata_abiotic=Coord
  ...
  table_prob2=plda[["posterior"]]
  vec=c()
  for(k in 1:9){
    pos=order(table_prob2[k,],decreasing=TRUE)[1]
    vec[k]=abiotic[pos]
  }
  match<-match+(vec == abiotic)
  }
   
     pander::pandoc.header(paste0(" **Matrice probabilitées : ",dim," dimensions**\n"), level = 3)
     cat("\n")
...
rownames(matrix)=c("Stress théorique","Circadian rythm","Photosynthesis","Drought",
       "Gamma","Other abiotic","Salt","Heavy Metal",
       "Nitrogen","Oxydative Stress","Temperature","UV")
...
############################Heatmap########################
 cat("\n")
  ...
  rownames(matrix_prob) <- c("Circadian rythm set","Photosynthesis set","Drought set",
       "Gamma set","Other abiotic set","Salt set","Heavy metal set","Nitrogen set","Oxydative stress set","Temperature set","UV set")
